...
const u8 TX_ADDRESS[TX_ADR_WIDTH]={0x 34,0x43,0x10,0x10,0x01}; //发送地址
...
void NRF24L01_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_AFIO,ENABLE);//AFIO是用于SPI复用必须开启
...
u8 NRF_Tx_Dat(u8 *txbuf)
{
	u8 sta;  
	NRF24L01_CE_0;
  NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,4);//写数据到TX BUF  4个字节
 	NRF24L01_CE_1;//启动发送	   
...
u8 NRF_Rx_Dat(u8 *rxbuf)
{
	...
	if(sta&RX_OK)//接收到数据
	{
		NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf,4);//读取数据
...
u8 NRF_RX_Mode(void)
{
  	...
  	NRF24L01_Write_Reg(WRITE_REG_G+RF_CH,40);	     //设置RF通信频率
...
{		
  	...
  	NRF24L01_Write_Reg(WRITE_REG_G+RF_CH,40);       //设置RF通道为40
  	NRF24L01_Write_Reg(WRITE_REG_G+RF_SETUP,0x0f);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
  	NRF24L01_Write_Reg(WRITE_REG_G+CONFIG,0x0e);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式,开启所有中断  	NRF24L01_Write_Reg(WRITE_REG_G+CONFIG,0x0e);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式,开启所有中断
